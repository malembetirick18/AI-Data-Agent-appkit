...
### Bug 51 — `$template` with bare `${fieldName}` inside `repeat` resolves to empty strings
`semantic_layer_api/src/signatures/genui_spec/genui_catalog_prompt.md`: the LLM generated `{"$template":"• ${third_party_id}: ${formatted}"}` inside a `repeat`-scoped element. `$template` only interpolates **absolute state paths** (`${/path}`) — bare names like `${third_party_id}` have no leading `/` and are not valid state paths, so they resolve to empty strings, producing `"• :"` bullet lines. Inside `repeat`, single item fields must be read with `{ "$item": "fieldName" }`. For compound strings (multiple fields concatenated), the only correct approach is to pre-format the full string as a dedicated `label` field in each state item in Python before emitting the state patch. Fixed: updated the catalog prompt with an explicit **NEVER** rule, a correct compound-string pattern, and a clarified `$template` entry in the Dynamic Props table.

### Bug 50 — Pie/Donut rendered as blank circle with high-cardinality or non-aggregated data
`client/src/components/InteractiveChart.tsx`: the `isReadyToRender` guard for pie/donut only checked `≥2 positive values`. Three additional cases produced a visually blank circle: (1) **too many data rows** — each row is one slice; 200 rows → 200 nearly-equal slivers → solid-looking circle (e.g. using a row-level `Id` column as `angleKey` against a categorical label with many rows); (2) **too many unique label categories** — unreadable legend + near-uniform arc widths; (3) **Radar with < 3 spokes** — geometrically degenerate (not polygon-shaped). Also added: **Line/Area must have ≥ 2 distinct X values** — a single point produces no path. Fixed: added `MAX_PIE_SLICES = 50` and `MAX_PIE_CATEGORIES = 20` module-level constants; extended `isReadyToRender` with per-chart guards covering all four cases.
